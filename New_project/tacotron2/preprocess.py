...
def process(speaker_path,
    ...
    Args:
        ...
        emotion_present: flag which indicates
        whether emotion is present in speaker data
    Returns:
        jobs: list of tuples to be processed by mapper
    """
    with open(os.path.join(speaker_path, 'metadata.csv'),
              'r',
              encoding='utf-8') as f:
        jobs = []
        ...
        #     file_name = file_name + '.wav'
            ...
            jobs.append((input_file_path, final_file_path, text, speaker_name,
                         speaker_id, emotion, process_audio))
...
def mapper(job):
    ...
    If speaker_data['process_audio'] == True,
    trims audio file and changes its format using ffmpeg.
    ...
    match = re.match('(.*)(.flac)', fout)
    fout = f'{match.group(1)}.wav'
    ...
    if process_audio:
        ...
        command = f'ffmpeg -y -i {fint} -acodec pcm_s16le -ac 1 -ar {Config.sr} {fout} -nostats -loglevel 0'
        os.system(command)
        os.remove(fint)
    else:
        duration = librosa.get_duration(data)
...
def balance_coefs(distribution, key):
    ...
    :param key:
    :return: dictionary with keys - dist[key].values,
            values - coefficients for loss balancing
    """
    true_balance = pd.DataFrame(distribution[key].value_counts() /
                                len(distribution))
    balance = pd.DataFrame({
        'true_balance': true_balance[key],
    ...
    balance['root'] = np.sqrt(
        np.divide(sum_sqrts, balance['sqrt_div_sum_sqrts']))
...
def main():
    ...
    if Config.start_from_preprocessed:
        distribution = pd.read_csv(os.path.join(Config.output_directory,
                                                'data.csv'),
                                   sep='|')
        ...
        for speaker_data in tqdm(Config.data):
            for speaker_path, dirs, files in os.walk(speaker_data['path']):
                if 'wavs' in dirs and 'metadata.csv' in files:
                    ...
                    sub_jobs = process(speaker_path, speaker_name, speaker_id,
        ...
        distribution['emotion_id'] = distribution['emotion'].map(
            Config.emo_id_map)

        distribution.to_csv(os.path.join(Config.output_directory, 'data.csv'),
                            sep='|',
                            index=False)
    ...
    if Config.limit_by in speakers:
        limiting_distribution = distribution[distribution['speaker_name'] ==
                                             Config.limit_by]
        ...
        print(f'Min {Config.limit_by} text: {mint}')
        print(f'Max {Config.limit_by} text: {maxt}')
        print(f'Min {Config.limit_by} dur: {mind}')
        print(f'Max {Config.limit_by} dur: {maxd}')
        print('----------------------------------------------')
    ...
    for speaker_data in Config.data:
        ...
        df = df[(df['text_len'] <= maxt) & (df['text_len'] >= 1) &
                (df['duration'] <= maxd) &
                (df['duration'] >= Config.minimum_viable_dur)]
        ...
        print(f'Train set for {speaker_name} is: {len(train)}')
        print(f'Val set for {speaker_name} is: {len(val)}')
        print('----------------------------------------------')
    ...
    with open(
            os.path.join(Config.output_directory, 'emotion_coefficients.json'),
            'w') as json_file:
        json.dump(e_coefs, json_file)

    s_coefs = balance_coefs(train, 'speaker_id')
    with open(
            os.path.join(Config.output_directory, 'speaker_coefficients.json'),
            'w') as json_file:
        json.dump(s_coefs, json_file)

    train.to_csv(os.path.join(Config.output_directory, 'train.txt'),
                 sep='|',
                 index=False,
                 header=False)
    val.to_csv(os.path.join(Config.output_directory, 'val.txt'),
               sep='|',
               index=False,
               header=False)
