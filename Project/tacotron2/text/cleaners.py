...
Cleaners are transformations that run over the input text at both training
and eval time.
Cleaners can be selected by passing a comma-delimited list of cleaner names
as the "cleaners"
hyperparameter. Some cleaners are English-specific. You'll typically want
to use:
...
_abbreviations = [(re.compile(fr'\b{x[0]}\.', re.IGNORECASE), x[1])
                  for x in [
    ('mrs', 'misess'),
    ('mr', 'mister'),
    ('dr', 'doctor'),
